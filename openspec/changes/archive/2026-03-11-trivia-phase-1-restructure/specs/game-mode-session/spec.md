...
The `gameMode` field SHALL be accessible from the WebSocket handler context so that incoming messages can be dispatched to mode-specific logic. In Phase 1 this branching is a skeleton only — trivia paths are stubs — but the field MUST be readable from handler scope.
